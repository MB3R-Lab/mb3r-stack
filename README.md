...
[![Candidate bundle](https://img.shields.io/badge/candidate-v0.3.1-orange)](https://github.com/MB3R-Lab/mb3r-stack/releases/tag/v0.3.1)
[![Upstream pins](https://img.shields.io/badge/Bering%20%7C%20Sheaft-0.3.2%20%7C%200.2.2-blue)](https://github.com/MB3R-Lab/mb3r-stack/blob/main/compat/stack-manifest.json)
...
Current bundle candidate pins published upstream artifacts explicitly: Bering `0.3.2` and Sheaft `0.2.2`, including immutable image digests and contract evidence in `compat/`.

Current packaged release assets for `v0.3.1` are:
...
The compatibility files in `compat/` are the source of truth for stack-level assertions. The current `0.3.1` bundle is recorded as a `candidate` integration statement backed by upstream release manifests, repository validation, release dry-run, adapter e2e, and Sheaft's published compatibility manifest, not as a broader verified operations guarantee. OpenTelemetry Demo remains one example profile and one acceptance scenario, not the design center of the core bundle.
